...
@dataclass(frozen=True)
class TenantId:
    """Identifies the customer/organization a task belongs to.

    Physical multi-tenant isolation (dedicated storage container/database
    per tenant) is implemented at the routing layer — see
    framework/multitenancy/context.py::TenantContext — not here; this type
    is just the stable identifier threaded through Task, memory keys,
    governance records, and per-tenant rate limiting.
    """
    value: str
...
class Task:
    ...
    # Which customer/organization this task belongs to. Optional today for
    # backward compatibility (single-tenant deployments, existing tests);
    # required for multi-tenant deployments — see framework/multitenancy.
    tenant_id:       Optional[TenantId] = None
    # Authenticated end-user identity distinct from tenant_id (R11) — set
    # from an IdentityAdapter-normalized Principal at the API boundary;
    # optional for backward compatibility with service-to-service callers.
    principal_id:    str = ""
    principal_roles: Tuple[str, ...] = field(default_factory=tuple)
    # Stable across retries (unlike task_id, which is regenerated by
    # for_retry()) — agents can key idempotency guards on this to avoid
    # duplicating side effects (e.g. memory writes) across retry attempts.
    idempotency_key: str = ""
    ...
    def create(
        ...
        **kwargs,
        ...
        tid = TaskId.generate()
        idempotency_key = kwargs.pop("idempotency_key", None) or str(tid)
        return cls(
            task_id=tid,
